...
namespace UIToolkitDemo
{
    ...
    public class SkillSO : ScriptableObject
    {
        ...
        // /// <summary>
        // /// Gets the display text describing the skill's damage over time.
        // /// </summary>
        // [CreateProperty]
        ...
        public string DamageText
        {
            get
            {
                return string.Format(m_LocalizedDamageText, DamagePoints, DamageTime);
            }
        }
        ...
        /// <summary>
        /// Updates the localized skill/category text.
        /// </summary>
        void OnValidate()
        {
            if (m_CategoryLocalized == null || m_CategoryLocalized.IsEmpty)
            {
                m_CategoryLocalized = new LocalizedString { TableReference = "SettingsTable" };
            }

            if (m_SkillNameLocalized == null || m_SkillNameLocalized.IsEmpty)
            {
                m_SkillNameLocalized = new LocalizedString { TableReference = "SettingsTable" };
            }
            
            if (m_TierTextLocalized == null || m_TierTextLocalized.IsEmpty)
            {
                m_TierTextLocalized = new LocalizedString 
                { 
                    TableReference = "SettingsTable",
                    TableEntryReference = "Skill.TierFormat" 
                };
            ...
            if (m_NextTierTextLocalized == null || m_NextTierTextLocalized.IsEmpty)
            {
                m_NextTierTextLocalized = new LocalizedString 
                { 
                    TableReference = "SettingsTable",
                    TableEntryReference = "Skill.NextTierFormat" 
                };
            }
            
            if (m_DamageTextLocalized == null || m_DamageTextLocalized.IsEmpty)
            {
                m_DamageTextLocalized = new LocalizedString 
                { 
                    TableReference = "SettingsTable",
                    TableEntryReference = "Skill.DamageFormat"
                };
